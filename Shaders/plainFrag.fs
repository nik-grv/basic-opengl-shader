...
float shinePower = 512;
float specularStrenght = 0.45;
...
{   
    ...
    norm = texture(normalTexture, uv).xyz;
    norm = (norm*2.0)-1.0;
...
vec3 GetDirectionalLight(vec3 norm, vec3 viewDir, float shadow) {
    ...
    vec3 result = ambientColor + (diffuseColor + blinnColor);
    return result;
}
...
vec3 GetPointLight(vec3 norm, vec3 viewDir)
{
    ...
    vec3 diffMapColor = texture(diffuseTexture, uv).xyz; 
    // vec3 specMapColor = vec3(texture(specularTexture, uv).x);// do i want this?
